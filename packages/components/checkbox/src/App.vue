...
export default defineComponent({
	...
	methods: {
		...
		handleCheckbox10Change(v: string[]) {
		...
		handleCheckbox11Change(v: boolean) {
			// 全选 or 全不选
			if (v) {
				(this.state
					.checkboxValue10 as string[]) = this.state.checkboxOriginValue10.map(
					innerV => {
						return innerV._id;
					}
				);
